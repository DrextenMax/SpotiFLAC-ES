...
> = {
  title: { label: "Título", template: "{title}" },
  "title-artist": { label: "Título - Artista", template: "{title} - {artist}" },
  "artist-title": { label: "Artista - Título", template: "{artist} - {title}" },
  "track-title": { label: "Pista. Título", template: "{track}. {title}" },
  "track-title-artist": {
    label: "Pista. Título - Artista",
    template: "{track}. {title} - {artist}",
  },
  "track-artist-title": {
    label: "Pista. Artista - Título",
    template: "{track}. {artist} - {title}",
  },
  "title-album-artist": {
    label: "Título - Artista del álbum",
    template: "{title} - {album_artist}",
  },
  "track-title-album-artist": {
    label: "Pista. Título - Artista del álbum",
    template: "{track}. {title} - {album_artist}",
  },
  "artist-album-title": {
    label: "Artista - Álbum - Título",
    template: "{artist} - {album} - {title}",
  },
  "track-dash-title": { label: "Pista - Título", template: "{track} - {title}" },
  "disc-track-title": {
    label: "Disco-Pista. Título",
    template: "{disc}-{track}. {title}",
  },
  "disc-track-title-artist": {
    label: "Disco-Pista. Título - Artista",
    template: "{disc}-{track}. {title} - {artist}",
  },
  custom: { label: "Personalizado…", template: "{title} - {artist}" },
};
...
export function FileManagerPage() {
    ...
    try {
      ...
      if (
        ...
        toast.error("No se pudieron cargar los archivos", {
          description: errorMsg || "Error desconocido",
        });
      }
      setAllFiles([]);
      setSelectedFiles(new Set());
  ...
  const handleShowMetadata = async (
    ...
    try {
      ...
      toast.error("No se pudieron leer los metadatos", {
        description: err instanceof Error ? err.message : "Error desconocido",
      });
  ...
  const handleShowLyrics = async (
    ...
    try {
      ...
      toast.error("No se pudo leer el archivo de letra", {
        description: err instanceof Error ? err.message : "Error desconocido",
      });
    }
  };

  const handleShowCover = async (
    filePath: string,
    e: React.MouseEvent,
  ) => {
    e.stopPropagation();
  ...
  const handleCopyLyrics = async () => {
    try {
      ...
      toast.error("No se pudo copiar la letra");
    }
  };
  ...
  const handleRename = async () => {
    ...
    try {
      ...
      if (successCount > 0)
        toast.success("Renombrado completado", {
          description: `${successCount} archivo(s) renombrado(s)${
            failCount > 0 ? `, ${failCount} con error` : ""
          }`,
        });
      else
        toast.error("Renombrado fallido", {
          description: `Los ${failCount} archivo(s) fallaron al renombrar`,
        });
          ...
          {!node.is_dir && (
            <>
              ...
              <Tooltip>
                ...
                <TooltipContent>Ver metadatos</TooltipContent>
              </Tooltip>
            </>
          )}
        </div>
    ...
    return nodes.map((node) => (
      <div key={node.path}>
        <div
          ...
          style={{ paddingLeft: `${depth * 16 + 8}px` }}
          onClick={(e) =>
            node.is_dir ? toggleExpand(node.path) : handleShowLyrics(node.path, e)
          }
        >
          {node.is_dir ? (
            <>
    ...
    return nodes.map((node) => (
      <div key={node.path}>
        <div
          ...
          style={{ paddingLeft: `${depth * 16 + 8}px` }}
          onClick={(e) =>
            node.is_dir ? toggleExpand(node.path) : handleShowCover(node.path, e)
          }
        >
          {node.is_dir ? (
            <>
  ...
  const allSelected =
    ...
    selectedFiles.size === allAudioFiles.length;

  return (
    <div className={`space-y-6 ${isFullscreen ? "h-full flex flex-col" : ""}`}>
      <div className="flex items-center justify-between shrink-0">
        <h1 className="text-2xl font-bold">Gestor de archivos</h1>
      </div>
      ...
      {activeTab === "track" && (
        <div className="space-y-2 shrink-0">
          <div className="flex items-center gap-2">
            <Label className="text-sm">Formato de renombrado</Label>
            <Tooltip>
              <TooltipTrigger asChild>
          ...
          <div className="flex items-center gap-2">
            ...
            <Tooltip>
              ...
              <TooltipContent>Restablecer predeterminado</TooltipContent>
            </Tooltip>
          </div>
      ...
      >
          ...
          <div className="flex items-center justify-between p-3 border-b bg-muted/30 shrink-0">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm text-muted-foreground">
                {selectedFiles.size} de {allAudioFiles.length} archivo(s)
                seleccionados
              </span>
            </div>
            <div className="flex items-center gap-2">
        ...
        <div
          className={`overflow-y-auto p-2 ${
            isFullscreen ? "flex-1 min-h-0" : "max-h-[400px]"
          }`}
        >
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              {rootPath
                ? `No se encontraron archivos de tipo ${activeTab}`
      ...
      <Dialog open={showResetConfirm} onOpenChange={setShowResetConfirm}>
        <DialogContent className="max-w-md [&>button]:hidden">
          <DialogHeader>
            <DialogTitle>¿Restablecer formato?</DialogTitle>
            <DialogDescription>
              Esto restablecerá el formato de renombrado a{" "}
              {"\"Título - Artista\""}. Tu formato personalizado se perderá.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            <Button onClick={resetToDefault}>Restablecer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showPreview} onOpenChange={setShowPreview}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col [&>button]:hidden">
          <DialogHeader>
            <DialogTitle>Vista previa de renombrado</DialogTitle>
            <DialogDescription>
              Revisa los cambios antes de renombrar. Los archivos con errores
              se omitirán.
            </DialogDescription>
          </DialogHeader>
              ...
              <>
                ...
                <Button onClick={handleRename} disabled={renaming}>
                  {renaming ? (
                    ...
                    <>
                      Renombrar{" "}
                      {previewData.filter((p) => !p.error).length} archivo(s)
                    </>
                  )}
                </Button>
              </>
      ...
      <Dialog open={showMetadata} onOpenChange={setShowMetadata}>
        <DialogContent className="max-w-md [&>button]:hidden">
          <DialogHeader>
            <DialogTitle>Metadatos del archivo</DialogTitle>
          ...
          {loadingMetadata ? (
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                <span className="text-muted-foreground">Artista del álbum</span>
              ...
              <div className="grid grid-cols-[100px_1fr] gap-2 text-sm">
                ...
                <span>
                  {metadataInfo.year
                    ? metadataInfo.year.substring(0, 4)
                    : "-"}
                </span>
              </div>
            </div>
          ...
          <DialogFooter>
            <Button onClick={() => setShowMetadata(false)}>Cerrar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog
        open={showLyricsPreview}
        onOpenChange={setShowLyricsPreview}
      >
        <DialogContent className="max-w-lg max-h-[80vh] overflow-hidden flex flex-col [&>button]:hidden">
          <DialogHeader>
            <DialogTitle>Vista previa de la letra</DialogTitle>
            <DialogDescription className="break-all">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => setShowLyricsPreview(false)}>
              Cerrar
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showCoverPreview} onOpenChange={setShowCoverPreview}>
        <DialogContent className="max-w-lg [&>button]:hidden">
          ...
          <DialogFooter>
            <Button onClick={() => setShowCoverPreview(false)}>
              Cerrar
            </Button>
          </DialogFooter>
        </DialogContent>
