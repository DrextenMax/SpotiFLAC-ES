        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-bold tracking-tight">Descargas</h2>
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setShowClearDownloadConfirm(true)}
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={downloadSortBy} onValueChange={setDownloadSortBy}>
              ...
              <SelectContent>
                ...
                <SelectItem value="date_asc">Fecha (antiguas primero)</SelectItem>
                <SelectItem value="title_asc">Título (A-Z)</SelectItem>
                <SelectItem value="title_desc">Título (Z-A)</SelectItem>
                <SelectItem value="artist_asc">Artista (A-Z)</SelectItem>
                <SelectItem value="artist_desc">Artista (Z-A)</SelectItem>
                <SelectItem value="duration_asc">Duración (corta)</SelectItem>
                <SelectItem value="duration_desc">Duración (larga)</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
          ...
          {paginated.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-16 text-center text-muted-foreground gap-3">
              ...
              <div className="space-y-1">
                <p className="font-medium text-foreground/80">Sin historial de descargas</p>
                <p className="text-sm">Tus canciones descargadas aparecerán aquí.</p>
              </div>
            </div>
          ) : (
            <table className="w-full table-fixed">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground hidden lg:table-cell w-32 text-xs uppercase">
                    Formato
                  </th>
                  <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground hidden xl:table-cell w-16 text-xs uppercase text-nowrap">
                    Dur
                  </th>
                  <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground hidden md:table-cell w-36 text-xs uppercase text-nowrap">
                    Fecha descarga
                  </th>
                  <th className="h-10 px-4 text-center align-middle font-medium text-muted-foreground w-32 text-xs uppercase text-nowrap">
                    Acciones
                  </th>
                </tr>
              </thead>
                  ...
                  <tr key={item.id} className="border-b transition-colors hover:bg-muted/50">
                    ...
                    <td className="p-3 align-middle min-w-0">
                      <div className="flex items-center gap-3 min-w-0">
                        <img
                          src={item.cover_url || "https://placehold.co/300?text=No+Cover"}
                          alt={item.album}
                    ...
                    <td className="p-3 align-middle text-left hidden lg:table-cell">
                      <div className="flex flex-col items-start gap-1">
                        <span className="text-xs font-bold text-foreground">
                          {["HI_RES_LOSSLESS", "LOSSLESS", "flac", "6", "7", "27"].includes(
                          ...
                          <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() => handlePreview(item.id, item.spotify_id)}
                                disabled={!item.spotify_id}
                              >
                            ...
                            <TooltipContent>
                              <p>{playingPreviewId === item.id ? "Pausar preview" : "Reproducir preview"}</p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                          ...
                          <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                className="h-8 w-8 cursor-pointer"
                                onClick={() =>
                                  openExternal(`https://open.spotify.com/track/${item.spotify_id}`)
                                }
                              >
            ...
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    if (downloadCurrentPage > 1) setDownloadCurrentPage(downloadCurrentPage - 1);
                  }}
                  className={
                    downloadCurrentPage === 1 ? "pointer-events-none opacity-50" : "cursor-pointer"
                  }
                />
              </PaginationItem>

              {getPaginationPages(downloadCurrentPage, totalPages).map((page, index) =>
                page === "ellipsis" ? (
                  ...
                  <PaginationItem key={page}>
                    <PaginationLink
                      ...
                      isActive={downloadCurrentPage === page}
              ...
              <PaginationItem>
                <PaginationNext
                  ...
                  className={
                    downloadCurrentPage === totalPages
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-bold tracking-tight">Consultas</h2>
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setShowClearFetchConfirm(true)}
          ...
          <div className="flex flex-col gap-4">
            <div className="flex gap-2 border-b shrink-0">
              <Button
                variant={activeFetchTab === "track" ? "default" : "ghost"}
                size="sm"
                onClick={() => setActiveFetchTab("track")}
              ...
              <Button
                variant={activeFetchTab === "album" ? "default" : "ghost"}
                size="sm"
                onClick={() => setActiveFetchTab("album")}
                className="rounded-b-none"
              >
              ...
              <Button
                variant={activeFetchTab === "playlist" ? "default" : "ghost"}
                size="sm"
                onClick={() => setActiveFetchTab("playlist")}
                className="rounded-b-none"
              >
              ...
              <Button
                variant={activeFetchTab === "artist" ? "default" : "ghost"}
                size="sm"
                onClick={() => setActiveFetchTab("artist")}
                className="rounded-b-none"
              >
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
                <Input
                  placeholder="Buscar en el historial de consultas..."
        ...
        <div className="rounded-md border overflow-hidden">
          {paginated.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground gap-3">
              ...
              <div className="space-y-1">
                <p className="font-medium text-foreground/80">Sin historial de consultas</p>
                <p className="text-sm">Los metadatos consultados aparecerán aquí.</p>
              </div>
            </div>
          ) : (
            <table className="w-full table-fixed">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground text-xs uppercase w-1/3">
                    {activeFetchTab === "artist" ? "Nombre" : "Título"}
                  </th>
                  <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground hidden md:table-cell text-xs uppercase">
                    Detalles
                  </th>
                  <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground hidden lg:table-cell w-40 text-xs uppercase text-nowrap">
                    Fecha consulta
                  </th>
                  <th className="h-10 px-4 text-center align-middle font-medium text-muted-foreground w-32 text-xs uppercase text-nowrap">
                    Acciones
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="h-10 w-10 rounded shrink-0 bg-secondary overflow-hidden">
                          {item.image ? (
                            ...
                            <div className="h-full w-full flex items-center justify-center text-xs text-muted-foreground font-medium bg-muted">
                          ...
                          <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                size="icon"
                                className="h-8 w-8 cursor-pointer text-destructive hover:text-destructive"
                                onClick={(e) => handleDeleteFetchItem(item.id, e)}
                              >
                                <Trash2 className="h-4 w-4" />
                              </Button>
                            </TooltipTrigger>
            ...
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    if (fetchCurrentPage > 1) setFetchCurrentPage(fetchCurrentPage - 1);
                  }}
                  className={
                    fetchCurrentPage === 1 ? "pointer-events-none opacity-50" : "cursor-pointer"
                  }
                />
              </PaginationItem>

              {getPaginationPages(fetchCurrentPage, totalPages).map((page, index) =>
                page === "ellipsis" ? (
                  <PaginationItem key={`ellipsis-${index}`}>
                    <PaginationEllipsis />
                  </PaginationItem>
                  ...
                  <PaginationItem key={page}>
                    <PaginationLink
                      ...
                      isActive={fetchCurrentPage === page}
                      className="cursor-pointer"
                    >
                      {page}
                    </PaginationLink>
    ...
    <div className="space-y-6">
      ...
      <div className="border-b">
        <div className="flex gap-6">
          <button
            ...
            className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px hover:text-foreground ${
              activeTab === "downloads"
      ...
      <Dialog open={showClearDownloadConfirm} onOpenChange={setShowClearDownloadConfirm}>
        <DialogContent className="max-w-md [&>button]:hidden">
          <DialogHeader>
            <DialogTitle>¿Borrar historial de descargas?</DialogTitle>
            <DialogDescription>
              Esto eliminará todas las entradas de tu historial de descargas. Esta acción no se
              puede deshacer.
              <br />
              Nota: los archivos descargados NO se eliminarán.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setShowClearDownloadConfirm(false)}
              className="cursor-pointer"
            >
              Cancelar
            </Button>
      ...
      <Dialog open={showClearFetchConfirm} onOpenChange={setShowClearFetchConfirm}>
        <DialogContent className="max-w-md [&>button]:hidden">
          <DialogHeader>
            <DialogTitle>
              ¿Borrar historial de {activeFetchTab.charAt(0).toUpperCase() + activeFetchTab.slice(1)}
              ?
            </DialogTitle>
            <DialogDescription>
              Esto eliminará todas las entradas de {activeFetchTab} de la caché de historial de
              consultas.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setShowClearFetchConfirm(false)}
              className="cursor-pointer"
            >
              Cancelar
            </Button>
            <Button
              variant="destructive"
              onClick={handleClearFetchHistory}
              className="cursor-pointer"
            >
              Borrar historial
            </Button>
