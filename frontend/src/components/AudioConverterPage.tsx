...
export function AudioConverterPage() {
  ...
  const isFormatDisabled =
    files.length > 0 && files.every((f) => f.format === "mp3");
    ...
    try {
      ...
      if (selectedFolder) {
        ...
        if (folderFiles && folderFiles.length > 0) {
          ...
          toast.info("No se encontraron archivos de audio", {
            description:
              "No se encontraron archivos FLAC o MP3 en la carpeta seleccionada.",
          });
        }
      }
    } catch (err) {
  ...
  const addFiles = useCallback(async (paths: string[]) => {
    ...
    if (m4aFiles.length > 0) {
      toast.error("Los archivos M4A no son compatibles", {
        description:
          "Solo se aceptan archivos FLAC y MP3 como entrada. Convierte los M4A primero.",
      });
    }
    ...
    const fileSizes =
      validPaths.length > 0 ? await GetFileSizes(validPaths) : {};
    ...
    setFiles((prev) => {
      ...
      if (newFiles.length > 0) {
        if (paths.length > newFiles.length) {
          ...
          toast.info("Algunos archivos se omitieron", {
            description: `${skipped} archivo(s) se omitieron (formato no compatible o ya añadidos)`,
          });
        }
      ...
      if (paths.length > 0 && m4aFiles.length === 0) {
        toast.info("No se añadieron archivos nuevos", {
          description:
            "Todos los archivos ya estaban añadidos o tienen un formato no compatible",
        });
      }
      return prev;
    });
  ...
  const handleConvert = async () => {
    if (files.length === 0) {
      toast.error("No hay archivos seleccionados", {
        description: "Añade archivos de audio para convertir.",
      });
      return;
    }
    ...
    try {
      ...
      if (successCount > 0) {
        toast.success("Conversión completada", {
          description: `Se convirtieron correctamente ${successCount} archivo(s)${
            failCount > 0 ? `, ${failCount} fallaron` : ""
          }`,
        });
      } else if (failCount > 0) {
        toast.error("La conversión ha fallado", {
          description: `Los ${failCount} archivo(s) fallaron al convertir`,
        });
      }
    } catch (err) {
      ...
      setFiles((prev) =>
        prev.map((f) => ({
          ...
          error: "La conversión ha fallado",
        }))
      );
    } finally {
      setConverting(false);
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Convertidor de audio</h1>
        {files.length > 0 && (
          ...
          <>
            ...
            <p className="text-sm text-muted-foreground mb-4 text-center">
              {isDragging
                ...
                : "Arrastra y suelta archivos de audio aquí, o usa los botones de abajo para seleccionarlos"}
            </p>
          ...
          <div className="w-full h-full p-6 space-y-4 flex flex-col">
            <div className="space-y-2 pb-4 border-b shrink-0">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  <Label className="whitespace-nowrap">Formato:</Label>
                  <ToggleGroup
                    type="single"
                    variant="outline"
                    value={outputFormat}
                ...
                {outputFormat === "m4a" && hasFlacFiles && (
                  <div className="flex items-center gap-2">
                    <Label className="whitespace-nowrap">Códec:</Label>
                    <ToggleGroup
                      type="single"
                      variant="outline"
                      value={m4aCodec}
            ...
            <div className="flex items-center justify-between shrink-0">
              <div className="text-sm text-muted-foreground">
                {files.length} archivo(s) • {successCount} convertido(s)
              </div>
            </div>
