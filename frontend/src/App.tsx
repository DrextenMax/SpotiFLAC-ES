...
import TotakekeImg from "@/assets/totakeke.png";

const HISTORY_KEY = "spotiflac_fetch_history";
const MAX_HISTORY = 5;

const EducationalScreen = ({ onAccept }: { onAccept: () => void }) => {
  return (
    <div className="h-screen w-screen flex items-center justify-center bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      <div className="max-w-lg w-full px-6 py-8 rounded-2xl bg-black/40 border border-white/5 shadow-xl space-y-6">
        <h1 className="text-2xl font-semibold tracking-tight">
          Aviso de uso educativo
        </h1>
        <p className="text-sm text-slate-300 leading-relaxed">
          Esta herramienta se ofrece únicamente con fines educativos y de
          investigación personal. No está afiliada ni respaldada por Spotify,
          Tidal, Qobuz, Amazon Music ni otras plataformas mencionadas.
        </p>
        <p className="text-xs text-slate-400 leading-relaxed">
          Al continuar, aceptas usar el programa de forma responsable, cumplir
          con las leyes de tu país y respetar los términos de servicio de cada
          plataforma. El desarrollador no se hace responsable de usos
          indebidos.
        </p>
...
function App() {
  ...
  const [pendingPageChange, setPendingPageChange] =
  ...
  const [isFFmpegInstalled, setIsFFmpegInstalled] = useState<boolean | null>(
    null,
  );
  ...
  useEffect(() => {
    const stored = localStorage.getItem("sf_accepted_terms");
    if (stored === "true") {
      setAccepted(true);
      setShowSplash(true);
  ...
  const initSettingsAndFFmpeg = useCallback(() => {
  ...
  useEffect(() => {
    if (accepted) {
      const cleanup = initSettingsAndFFmpeg();
      return () => {
        if (typeof cleanup === "function") cleanup();
      };
    }
  }, [accepted, initSettingsAndFFmpeg]);

const handleAcceptEducational = () => {
  localStorage.setItem("sf_accepted_terms", "true");
  setAccepted(true);
  setShowSplash(true);
  ...
  const handleEnableSpotFetchApi = async () => {
    try {
      ...
      toast.success(
        "SpotFetch API activada. Ahora puedes volver a intentar la búsqueda.",
      );
    } catch (err) {
  ...
  const handleInstallFFmpeg = async () => {
    ...
    try {
      ...
      if (response.success) {
        toast.success("FFmpeg instalado correctamente.");
        setIsFFmpegInstalled(true);
      } else {
        toast.error(`Error al instalar FFmpeg: ${response.error}`);
      }
    } catch (error) {
      console.error("Error installing FFmpeg:", error);
      toast.error(`Error durante la instalación de FFmpeg: ${error}`);
    } finally {
  ...
  useEffect(() => {
    ...
    if ("track" in metadata.metadata) {
      ...
      historyItem = {
        ...
        artist: `${album_info.total_tracks.toLocaleString()} pistas`,
      ...
      historyItem = {
        ...
        artist: `${playlist_info.tracks.total.toLocaleString()} pistas`,
        image:
          playlist_info.cover || playlist_info.owner.images || "",
      };
      ...
      historyItem = {
        ...
        artist: `${artist_info.total_albums.toLocaleString()} álbumes`,
  ...
  const toggleSelectAll = (tracks: any[]) => {
    const tracksWithId = tracks
      .filter((track) => track.spotify_id)
      .map((track) => track.spotify_id || "");
    if (tracksWithId.length === 0) return;
    const allSelected = tracksWithId.every((id) =>
      selectedTracks.includes(id),
    );
    if (allSelected) {
      setSelectedTracks((prev) =>
        prev.filter((id) => !tracksWithId.includes(id)),
      );
    } else {
      setSelectedTracks((prev) =>
        Array.from(new Set([...prev, ...tracksWithId])),
      );
    }
  };
  ...
  const handleOpenFolder = async () => {
    ...
    if (!settings.downloadPath) {
      toast.error("Ruta de descarga no configurada");
      return;
    }
    try {
      ...
      toast.error(`Error al abrir la carpeta: ${error}`);
    }
  };
    ...
    if ("track" in metadata.metadata) {
      ...
      return (
        <TrackInfo
          ...
          isDownloaded={download.downloadedTracks.has(trackId)}
          isFailed={download.failedTracks.has(trackId)}
          isSkipped={download.skippedTracks.has(trackId)}
          downloadingLyricsTrack={lyrics.downloadingLyricsTrack}
          downloadedLyrics={lyrics.downloadedLyrics.has(
            track.spotify_id || "",
          )}
          failedLyrics={lyrics.failedLyrics.has(track.spotify_id || "")}
          skippedLyrics={lyrics.skippedLyrics.has(
            track.spotify_id || "",
          )}
          checkingAvailability={
            availability.checkingTrackId === track.spotify_id
          }
          availability={availability.availabilityMap.get(
            track.spotify_id || "",
          )}
          downloadingCover={
            cover.downloadingCoverTrack ===
            (track.spotify_id || `${track.name}-${track.artists}`)
          }
          downloadedCover={cover.downloadedCovers.has(
            track.spotify_id || `${track.name}-${track.artists}`,
          )}
          failedCover={cover.failedCovers.has(
            track.spotify_id || `${track.name}-${track.artists}`,
          )}
          skippedCover={cover.skippedCovers.has(
            track.spotify_id || `${track.name}-${track.artists}`,
          )}
    ...
    if ("album_info" in metadata.metadata) {
      ...
      return (
        <AlbumInfo
          ...
          onDownloadAll={() =>
            download.handleDownloadAll(track_list, album_info.name, true)
          }
          onDownloadSelected={() =>
          ...
          <>
            ...
            >
              <DialogContent className="sm:max-w-[425px] p-6 [&>button]:hidden">
                ...
                <DialogDescription>
                  ¿Quieres obtener metadatos para este álbum?
                </DialogDescription>
                ...
                <DialogFooter>
                  ...
                  <Button
                    onClick={async () => {
                      const albumUrl =
                        await metadata.handleConfirmAlbumFetch();
                      if (albumUrl) {
  ...
  if (!accepted) {
    return <EducationalScreen onAccept={handleAcceptEducational} />;
  }
  ...
  return (
    <TooltipProvider>
      <div className="min-h-screen bg-background flex flex-col">
        ...
        >
          <DialogContent className="sm:max-w-[425px] [&>button]:hidden">
            <DialogHeader>
              ...
              <DialogDescription>
                Tienes cambios sin guardar en Ajustes. ¿Seguro que quieres
                salir? Tus cambios se perderán.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
          ...
          <DialogContent className="max-w-[360px] [&>button]:hidden p-6 gap-5">
            <DialogHeader className="space-y-2">
              ...
              <DialogDescription className="text-sm text-foreground/70 leading-relaxed font-normal">
                FFmpeg es esencial para que SpotiFLAC funcione correctamente.
                Esta configuración descargará alrededor de{" "}
                    ...
                    <div className="flex justify-between text-[11px] font-bold">
                      <div className="flex flex-col gap-0.5">
                        ...
                        {downloadProgress.is_downloading &&
                          downloadProgress.mb_downloaded > 0 && (
            ...
            <DialogFooter className="flex-row gap-3 pt-2">
              ...
              <Button
                className={`${
                  isInstallingFFmpeg ? "w-full" : "flex-1"
                } h-11 text-sm font-bold shadow-lg shadow-primary/10`}
                onClick={handleInstallFFmpeg}
                disabled={isInstallingFFmpeg}
              >
                {isInstallingFFmpeg ? "Instalando…" : "Instalar ahora"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog
          open={metadata.showApiModal}
          onOpenChange={metadata.setShowApiModal}
        >
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>SpotFetch API recomendada</DialogTitle>
              <DialogDescription>
                La obtención directa ha fallado. Esto suele ocurrir cuando tu{" "}
                <span className="text-foreground font-bold">
                  país está bloqueado
                </span>{" "}
                por Spotify o tu IP está restringida. ¿Quieres activar la{" "}
                <span className="text-foreground font-bold">
                  SpotFetch API
                </span>{" "}
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => metadata.setShowApiModal(false)}
              >
                Cancelar
              </Button>
              <Button onClick={handleEnableSpotFetchApi}>
                Activar SpotFetch API
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>

        <div className="w-full flex justify-end px-4 py-2 text-xs text-muted-foreground">
          Fork by J.Sanchez
        </div>
      </div>
    </TooltipProvider>
